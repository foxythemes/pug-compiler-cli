...
options.conf = ( options.conf == undefined || options.conf == null ) ? './src/dev-config.json' : options.conf; 

// Read config files
var conf = JSON.parse( fs.readFileSync( options.conf, 'utf8' ) );
var env_conf = JSON.parse( fs.readFileSync( './src/' + options.env + '-config.json', 'utf8' ) );
