...
const optionDefinitions = [
  { name: 'conf-path-main', alias: 'c', type: String },
  { name: 'env', alias: 'e', type: String },
  { name: 'conf-path-env', alias: 'x', type: String },
  { name: 'views-path', alias: 'v', type: String },
  { name: 'dest-path', alias: 'd', type: String }
];
...
// Read config files
var conf = JSON.parse( fs.readFileSync( options.confPathMain, 'utf8' ) );
var env_conf = JSON.parse( fs.readFileSync( options.confPathEnv, 'utf8' ) );

var path = {
  basedir: options.viewsPath,
  basedirDist: options.destPath
}
...
function generateHmtl( name ){
	...
	fs.writeFile(path.basedirDist + name +'.'+ conf.pagesExtension, html, function( err ) {
	  if( err ){
	  	...
	  	console.log(chalk.green(' [pugtohtml]: ' + name + "." + conf.pagesExtension + " was generated" ));
	  }	  
	});	
}